...
public class ProductApprovingActivity extends AppCompatActivity
{
    Button B;
    ImageView IV;
    DrawerLayout D;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.product_approving_activity);
        ...
        CropCronyDatabase DB = new CropCronyDatabase(ProductApprovingActivity.this);

        D = (DrawerLayout) findViewById(R.id.productApprovingDrawer);
        ...
        B = (Button) findViewById(R.id.productApprovingButton);

        IV = (ImageView) findViewById(R.id.productApprovingImageView);

        TV = (TextView) findViewById(R.id.productApprovingTextView1);
        TV2 = (TextView) findViewById(R.id.productApprovingTextView2);
        TV3 = (TextView) findViewById(R.id.productApprovingTextView4);
        TV4 = (TextView) findViewById(R.id.productApprovingTextView6);
        TV5 = (TextView) findViewById(R.id.productApprovingTextView8);
        TV6 = (TextView) findViewById(R.id.productApprovingTextView10);
        ...
        {
            Toast.makeText(ProductApprovingActivity.this, "Product not found!", Toast.LENGTH_SHORT).show();
        }
        else
        {
        ...
        Cursor Bidder = DB.GetDataById("BiddingsTable", "BiddingProduct", TV.getText().toString());
        if(Bidder.getCount() == 0)
        {
            Toast.makeText(this, "No one bidden yet.", Toast.LENGTH_SHORT).show();
        }
        else
        {
            while (Bidder.moveToNext())
            {
                String highestBidder = Bidder.getString(Bidder.getColumnIndex("HighestBidder"));
                TV5.setText(highestBidder);
            }
        }

        Cursor Status = DB.GetDataById("ProductsTable", "ProductId", ProductId);
        while (Status.moveToNext())
        {
            String status = Status.getString(Status.getColumnIndex("ProductStatus"));
            TV6.setText(status);
        }
        ...
        B.setOnClickListener(new View.OnClickListener()
        {
            ...
            {
                boolean Sold = DB.Update( ProductId, "Sold");
                boolean Approve = DB.Insert("AuctionedProductTable", TV.getText().toString(), TV5.getText().toString(), TV4.getText().toString(), "");
                if (Sold)
                {
                    Toast.makeText(ProductApprovingActivity.this, "Product Sold!, Thank you.", Toast.LENGTH_SHORT).show();
                }
                else
                {
                    Toast.makeText(ProductApprovingActivity.this, "Product Approving Failed, please try again later.", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
