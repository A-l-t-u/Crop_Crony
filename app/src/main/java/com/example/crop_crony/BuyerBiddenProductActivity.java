...
public class BuyerBiddenProductActivity extends AppCompatActivity
{
    DrawerLayout D;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.buyer_bidden_product_activity);
        ...
        CropCronyDatabase DB = new CropCronyDatabase(BuyerBiddenProductActivity.this);

        RecyclerView RV = (RecyclerView) findViewById(R.id.buyerBiddenRecyclerView);

        Button B2 = (Button) findViewById(R.id.buyerBiddenButton2);

        D = (DrawerLayout) findViewById(R.id.buyerBiddenDrawer);
        ...
        LinearLayoutManager LLM = new LinearLayoutManager(BuyerBiddenProductActivity.this);
        RV.setLayoutManager(LLM);
        ArrayList<ProductsClass> ProductData = new ArrayList<ProductsClass>();
        ProductData.clear();
        Cursor buyerProductData = DB.GetDataById("BiddingsTable", "HighestBidder", UserName);
        if (buyerProductData.getCount() == 0)
        {
            Toast.makeText(BuyerBiddenProductActivity.this, "No data found!", Toast.LENGTH_SHORT).show();
        }
        else
        {
            while (buyerProductData.moveToNext())
            {
                Cursor productsData = DB.GetDataById("ProductsTable", "ProductName", buyerProductData.getString(buyerProductData.getColumnIndex("BiddingProduct")));
        ...
        BuyerMainAdapter productData = new BuyerMainAdapter(BuyerBiddenProductActivity.this, ProductData);
